'''This code is written to statistically downscale minimum daily temperature from global climate output (here CMIP5), using a 39 year gridded dataset (here bias-corrected WRF) in place of observations, using the method described in Potter et al., 2022 following Cannon et al., 2014.
...
outputfile="output_files_QM_tmin45_"+str(int(job)+1)+".txt"
...
xr_wrf_min=xr.open_dataset('filepath_to_netcdf.nc')

#make sure the gridded dataset is in the required format
xr_wrf_fixed_min=xr.Dataset(data_vars={'T2':(['Time','lat','lon'],xr_wrf_min.T2.data)},coords={'lon':(['lon'],xr_wrf_min.lon[0,:].data),'lat':(['lat'],xr_wrf_min.lat[:,0].data),'Time':xr_wrf_min.Time.data,'month':xr_wrf_min['Time.month'].data}).T2
...
#open the historical CMIP5 model, which has been regridded to the WRF resolution
xr_hist_wrfres['tasmin']=xr_hist_wrfres.tasmin-273.15 #Kelvin to Celsius
...
xr_fut_wrfres['tasmin']=xr_fut_wrfres.tasmin-273.15 #Kelvin to Celsius
...
#########################################
final_corrected_future_to100=correct_full_timeseries_parallel(xr_wrf_fixed_min,xr_hist_wrfres.tasmin,xr_fut_wrfres_to100.tasmin)
